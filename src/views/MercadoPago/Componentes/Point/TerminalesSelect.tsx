...
import RefreshButton from "../../../Componentes/Buttons/RefreshButton";
...
export default function TerminalesSelect({
  ...
  const { terminalId, setTerminalId, shouldPrint, setShouldPrint } =
    useMercadoPagoStore();
  ...
  const fetchTerminales = async () => {
    ...
    try {
      ...
      if (
        !isControlled &&
        currentSelectedId &&
        !rows.find((t) => t.id === currentSelectedId)
      ) {
        setTerminalId(null);
      }
    } catch (e: any) {
  ...
  const cambiarModo = async (t: TerminalLite) => {
    ...
    try {
      ...
      if (!res.ok) {
        pushError("pointChangeOperatingMode", {
          id: requestId,
          httpStatus,
          durationMs,
          request: {
            url: urlPostCambiarModo,
            method: "POST",
            headers: {
              "Content-Type": "application/json",
              Accept: "application/json",
          ...
          errorMessage: `HTTP ${httpStatus} - ${
            (json as any)?.message ?? "Error al cambiar modo"
          }`,
          meta: { note: `Terminal ${t.external_pos_id || t.id}` },
        });
        throw new Error(`HTTP ${httpStatus}`);
      }

      pushSuccess("pointChangeOperatingMode", {
        id: requestId,
        httpStatus,
        durationMs,
        request: {
      ...
      // Actualizamos la UI local: marcamos PDV (o tomá del json si viene)
      setTerminales((prev) =>
        (prev ?? []).map((x) =>
      ...
      pushError("pointChangeOperatingMode", {
        ...
        errorMessage: e?.message || "Error desconocido al cambiar modo",
        meta: { note: `Terminal ${t.external_pos_id || t.id}` },
      });
              ...
              {isStandalone && !hideChangeModeButton && (
                <div className="mt-2">
                  <button
                    ...
                    className="inline-flex items-center gap-2 rounded-md bg-blue-600 text-white px-3 py-1.5 text-xs font-medium shadow-sm hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
