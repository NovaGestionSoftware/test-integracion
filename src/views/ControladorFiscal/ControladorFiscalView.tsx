...
function buildUrl(ip: string, path: string, payload: object) {
  const host = ip.startsWith("http") ? ip.replace(/\/+$/, "") : `http://${ip}`;
  const qp = `_i=${encodePayload(payload)}`;
  return `${host}:4001${path}?${qp}`;
}

function isValidIPv4OrHost(value: string) {
  const ipv4 =
...
export default function ControladorFiscalView() {
  ...
  const ipValida = useMemo(() => isValidIPv4OrHost(ip.trim()), [ip]);
  ...
  const handleObtenerInfo = async () => {
    ...
    try {
      ...
      setErrorMsg(
        err instanceof Error ? err.message : "Error desconocido al llamar al endpoint"
      );
      setInfoResp(null);
    } finally {
  ...
  const handleObtenerVersion = async () => {
    ...
    try {
      ...
      setErrorMsg(
        err instanceof Error ? err.message : "Error desconocido al llamar a versión"
      );
      setVerResp(null);
    } finally {
      setLoadingVer(false);
  ...
  return (
    <div className="p-4 md:p-6">
      ...
      <div className="mt-4 rounded-2xl border border-zinc-200 bg-white p-4">
        <label className="text-sm font-medium">IP pública del equipo</label>
        <input
          value={ip}
          onChange={(e) => setIp(e.target.value)}
          placeholder="Ej: 200.110.90.10 ó mi-dominio.com"
        ...
        {!ipValida && ip.length > 0 && (
          <p className="mt-1 text-xs text-red-600">
            Ingresá una IP v4 o un hostname válido.
          </p>
        )}
        ...
        <div className="mt-3 space-y-1 text-xs text-zinc-600">
          <div className="flex items-start gap-2">
            ...
            <div className="overflow-hidden">
              ...
              {urlInfoPreview ? <span className="break-all">{urlInfoPreview}</span> : <span>—</span>}
            </div>
          </div>
          <div className="flex items-start gap-2">
            <FiLink2 className="mt-0.5 shrink-0" />
      ...
      {/* Resultados provisorios (hasta enchufar InspectorPanel) */}
      ...
      {infoResp?.raw
        ? String(infoResp.raw) // si viene texto crudo, mostrar directo con saltos
        : JSON.stringify(infoResp ?? {}, null, 2)}
    </pre>
        </div>
